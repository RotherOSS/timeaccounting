...
# --
# $origin: otobo - c4729719b9788da163909113055de495fac35df4 - Kernel/System/Ticket/Article.pm
# --
...
Update a Ticket TimeUnit entry.

    my $Success = $ArticleObject-ArticleAccountedTimeUpdate(
        TicketID    => 1,
        ArticleID   => 1,
        UserID      => 1
    );
...
sub ArticleAccountedTimeUpdate {
    my ( $Self, %Param ) = @_;
    ...
    # Get old timeunit
    $Param{OldTimeUnit} = $Self->ArticleAccountedTimeGet(
        ArticleID => $Param{ArticleID},
    );

    # If old and new is the same, exit
    if ( $Param{NewTime} eq $Param{OldTimeUnit} ) {
        return 1;
    }
    ...
    # db update
    return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
        SQL => "UPDATE time_accounting SET time_unit = ?, change_time = current_timestamp, change_by = ? WHERE article_id = ?",
        Bind => [ \$Param{NewTime}, \$Param{UserID}, \$Param{ArticleID} ],
    );
    ...
    $Kernel::OM->Get('Kernel::System::Ticket')->HistoryAdd(
        ...
        HistoryType  => 'Misc',
        Name         => "User $Param{UserID} update article $Param{ArticleID}. Old timeunits $Param{OldTimeUnit}, new $Param{NewTime}",
    );
